...
def flip(image,vertical=False):
    ...
    if vertical == False:
        for col in range(int(width/2)):
            for row in range(height):
                pixelOrr = image[row][col]
                pixeltarg = image[row][width-1-col]
                image[row][col] = pixeltarg
                image[row][width-1-col] = pixelOrr
    elif vertical == True:
        for row in range(int(height/2)):
            for col in range(width):
                pixelOrr = image[row][col]
                pixeltarg = image[height-1-row][col]
                image[row][col] = pixeltarg
                image[height-1-row][col] = pixelOrr
    return True
