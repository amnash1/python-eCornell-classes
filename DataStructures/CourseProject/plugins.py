...
def mono(image, sepia=False):
    ...
    for row in image:
        ...
        for pix in row:
            #print(pix)
            brightness = int(0.3 * pix.red + 0.6 * pix.green + 0.1 * pix.blue)

            if sepia == True:
                pix.red = brightness
                pix.green = int(brightness * 0.6)
                pix.blue = int(brightness * 0.4)

            elif sepia == False:
                pix.red = brightness
                pix.green = brightness
                pix.blue = brightness
...
def flip(image,vertical=False):
    ...
    #iterate the hight/width of the image
    #debug.png is 2x2 (or (0,1)(0,1)
    width = len(image[0])
    #print(width)
    height = len(image)
    ...
    if vertical ==True:
        ...
        for yaxis in range(width):
            for xaxis in range(height//2):
                #top = image[xaxis][yaxis]
                toBottom = image[height - 1 - xaxis][yaxis]
                #image[xaxis][yaxis] = bottom
                #image[height - 1 - xaxis][yaxis]= top
                image[height - 1 - xaxis][yaxis] = image[xaxis][yaxis]
                image[xaxis][yaxis] = toBottom

    else:
        print("It is Horizontal")
        for xaxis in range(height):
            for yaxis in range(width//2):
                    #print ('y axis row being evauluated '+str(yaxis))
                #copies left to right
                left2Right = image[xaxis][width - yaxis -1]
                image[xaxis][width - yaxis -1] = image[xaxis][yaxis]
                    #left2Right done

                #copies left to right
                image[xaxis][yaxis] = left2Right
                #right2Left done
...
def transpose(image):
    ...
    for xaxis in range(numcols):

        # Make a single row
        row = [] # Single row accumulator
        for yaxis in range(numrows):
            row.append(image[yaxis][xaxis])

        #Add the result to the table
        result.append(row)

    #Then remove all the rows in the image and replace it with
    #the rows from the transposed copy.
    image [:] = result
...
def rotate(image,right=False):
    ...
    """
    Read the specification for the function rotate.
    This function has an optional parameter right, which is False by default
    (meaning the image will rotate to the left). Implement this function assuming
    that right is False. You do not need to add an if-statement yet.
    You can delay that until the next step.
    As you can see from the specification, rotation is easy now that you have
    implemented both flip and transpose. To rotate to the left,
    transpose first and then flip vertically. For example, the command
    """

    # We recommend enforcing the precondition for right
    # Important: Because you can put anything you want in the right option
    #(say --right=blue), we recommend that you enforce the precondition for
    #right in the function with assert statements.
    ...
    if right == True:
        ...
        #To rotate to the left,transpose first
        #call to transpose
        transpose(image)
